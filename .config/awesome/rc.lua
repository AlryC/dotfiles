...
local function not_mono(str)
	return '<span font_family="FiraCode Nerd Font">' .. str .. "</span>"
end
--- }}}
...
awful.screen.connect_for_each_screen(function(s)
	...
	s.mybattery:buttons(gears.table.join( -- add turning pc off and on rofi widget on left click
		...
		awful.button({}, 3, function()
			s.mybattery.hidden = not s.mybattery.hidden
			update_battery()
		end)
	))
	...
	s.mywifi:buttons(gears.table.join( -- add wifi select rofi widget on left click
		awful.button({}, 1, function()
			os.execute(home_path .. ".assets/scripts/sb-wifi-select.sh")
		end),
		awful.button({}, 3, function()
	...
	s.mybluetooth:buttons(gears.table.join( -- add bluetooth rofi widget on left click
		--awful.button({ }, 1, function () awful.layout.inc( 1) end),
		awful.button({}, 3, function()
			s.mybluetooth.hidden = not s.mybluetooth.hidden
			update_bluetooth()
		end)
	))
	-- Timer to refresh battery info every 5s
...
globalkeys = gears.table.join(
	...
	--awful.key({}, "F4", awful.tag.history.restore, { description = "switch to last tag (not window)", group = "tag" }),
	...
	awful.key({ modkey }, "p", function()
		os.execute("dmenu_run -hp arduino,anki,librewolf,obsidian,telegram,code,stm32cubeide,stm32cubeprg,idea")
	end, { description = "Run dmenu", group = "launcher" }),
	-- Custom
	-- Toggle Modes
	awful.key({ modkey }, "f", function()
		awful.screen.focused().selected_tag.layout = awful.layout.suit.floating
	end, { description = "set floating layout for tag", group = "client" }),
	awful.key({ modkey }, "t", function()
		awful.screen.focused().selected_tag.layout = awful.layout.suit.tile
	end, { description = "set tiling layout for tag", group = "client" }),
	...
	awful.key({ modkey, "Shift" }, "w", function()
		os.execute("~/suckless/assets/scripts/dt-wallpaper-select.sh")
	end, { description = "set tiling layout for tag", group = "client" }),
	-- Take a screenshot
	awful.key({}, "Print", function()
		os.execute("maim --select | xclip -selection clipboard -t image/png")
...
client.connect_signal("manage", function(c)
	...
	if
		not awesome.startup
		and (c.floating or awful.layout.get(c.screen) == awful.layout.suit.floating)
		and not c.maximized
